import { Body, Controller, Delete, Get, HttpException, HttpStatus, Inject, Param, Patch, Post, Session, UseGuards, UsePipes, ValidationPipe } from '@nestjs/common';
...
export class TodoController {
    ...
    async deleteFolder(@Session() session: Record<string, any>, @Body() deleteFolderDto: DeleteFolderDto) {
        if (session.passport.user.username === deleteFolderDto.owner)
            return await this.todoService.deleteFolderByName(deleteFolderDto);
        else
            throw new HttpException("Unauthorized access", HttpStatus.UNAUTHORIZED);
    ...
    @Get('folder/:id')
    @UsePipes(ValidationPipe)
    async getTasks(@Session() session: Record<string, any>, @Param('id') folder: string)  {
        console.log(folder);
        return await this.todoService.getTasks(folder, session.passport.user.username);
    }
